...
public class TypeChecker {
    ...
    static Type Check(For f, TypeEnv te) {
        addType(f.decl, te);
        Type t2 = Check(f.expr, te);
        Type t3 = Check(f.assignment, te);
        Type t4 = Check(f.stmt, te);
    ...
    public static TypeEnv addType (Decl d, TypeEnv te) {
        // put the variable decls into a symbol table(TypeEnv)
        if (d != null)
            Check(d, te);
        return te;
    ...
    static TypeEnv deleteType(Decl d, TypeEnv te) {
        if(d != null) {
            te.pop();
        }
        return te;
